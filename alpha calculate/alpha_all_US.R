alpha_all_US_withimm<-function(gatherdata,m,outputpath,Y1NAME,Y0NAME){
  gatherdata <- gatherdata[complete.cases(gatherdata),]
  #options(mc.cores = parallel::detectCores())
  rstan_options(auto_write = TRUE)
  set.seed(1111)
  policy_variable <- c("School.closing", "Workplace.closing", "Close.Public.transport",
                       "Facial.Coverings","Restrict.Movement","Restrict.Gathering")
  state_variable <- c("HealthIndex", "PopDensity", "Aging")
  ...
  fit<-rstan::sampling(object = m,data=dataset,iter=2000,warmup=500,
                       chains=4,thin=1,control = list(adapt_delta = 0.95, max_treedepth =12))
